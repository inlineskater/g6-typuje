...
- `supabase/farm.sql` — „Farma" (Phase 1): a shared **10×4** owned-tile grid + collectible plant-card economy exposed as **🌱 Ogródek** (the old zen `water_plant` tab is hidden from nav but its plant/pot code is still reused for migrated plant tiles). Adds `farm_tiles` (sparse, public SELECT, PK `(x,y)`), `farm_card_defs` (seeded catalog), `farm_collection`/`farm_inventory` (own-row SELECT), `farm_market` (dynamic NPC prices, public SELECT), `farm_nft_instances` (serialized NFT cards, public SELECT), `farm_user_state` (own-row SELECT — owned sealed `boxes` + `tile_vouchers` + one-time starter-gift flags); realtime on tiles/market/collection/inventory/user_state; and SECURITY DEFINER RPCs `buy_farm_tile` (claim via `INSERT ... ON CONFLICT DO NOTHING`; consumes a free-tile voucher if held, else escalating BURN), `claim_farm_starter` (idempotent one-time welcome gift: **3 free boxes** + a tile voucher **guaranteed within those first 3 openings**), `buy_farm_lootbox` (BURN 100/box → sealed boxes), `open_farm_lootbox` (consumes one owned box, no coin cost; weighted `random()` draw of **3 distinct** cards → `cards[]`, plus ~7% bonus `{voucher:true}` free-tile drop), `level_up_card` (consume dupes + coins BURN), `plant_crop`/`harvest_crop` (server-timed growth, crops minted to inventory; `plant_crop` rejects `acquired_via='migration'` plant tiles → `zen_tile`), `sell_crop_to_npc` (row-locked dynamic price, coins MINTED). Boxes are **bought first (Sklep), opened later (Ekwipunek)** — two-step. Idempotent; includes a run-once migration placing each existing gardens row (linked by `zen_garden_id`) on one **plant tile** (`acquired_via='migration'`, plant-only — delete+recreate, deterministic). **Bounds (10×4) mirror `FARM_W`/`FARM_H` in `index.html`; coin reasons `farm_tile_buy`/`farm_box_buy`/`card_levelup` (burn) and `farm_crop_sale` (mint) must stay in sync with `economy-stats.sql` + `leaderboard-net-worth-items.sql` (which also value owned boxes ×100 + vouchers ×350 in `farm_assets`)**
- `supabase/farm-price-history.sql` — „Farma": per-crop NPC price history for the **📈 Ceny** hub tab. Adds `farm_price_history` (public SELECT), a trigger logging every `farm_market.cur_price` change, `snapshot_farm_prices()` (effective-price snapshot, scheduled every 6h via pg_cron), and a one-time backfill from past `farm_crop_sale` coin-ledger rows. Frontend: `buildFarmMarketBody`/`farmPriceChart` (inline SVG line chart per crop).
