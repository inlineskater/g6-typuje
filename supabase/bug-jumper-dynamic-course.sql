-- Bug Jumper: Dynamic Course relaunch (2026-07-27).
--
-- The old "Hard Course" (course_id = 'bug_jumper_hard_v2') was one fixed,
-- identical-for-everyone 30-line board. This migration switches Bug Jumper to
-- a procedurally generated course: each round draws a random seed, and both
-- the browser and bug-jumper-action derive the exact same lane layout AND
-- corridor shape (straight / L-turn / S-curve / zigzag — see BJ_SHAPES in
-- index.html) from that seed via the shared bjGenerateCourse(seed) algorithm.
-- Walls now block parts of every non-safe row, so climbing is no longer pure
-- "always jump up" — you have to shuffle sideways to stay inside the
-- corridor before advancing a row.
--
-- Run after supabase/bug-jumper.sql. Idempotent.
--
-- ⚠️ Keep bjGenerateCourse/bjBugColAt/bjCellBlocked/bjCellOpen in index.html
--    and generateCourse/bugColAt/cellBlocked/cellOpen in
--    supabase/functions/bug-jumper-action/index.ts byte-for-byte identical —
--    the server replay depends on it.

ALTER TABLE public.bug_jumper_rounds
  ADD COLUMN IF NOT EXISTS seed integer NOT NULL DEFAULT 0;

-- ⚠️ SUPERSEDED by this file: bug-jumper.sql's copies of these three views
-- and award_bug_jumper_week filter on course_id = 'bug_jumper_hard_v2'. Old
-- Hard Course rows stay in bug_jumper_scores/bug_jumper_weekly_awards as
...
round_counts AS (
  ...
  WHERE course_id = 'bug_jumper_dynamic_v1'
