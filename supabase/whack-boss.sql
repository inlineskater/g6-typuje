-- Whack-a-Boss support for Rynek Proroctw G6.
-- Run after supabase/schema.sql on an existing project.

CREATE OR REPLACE FUNCTION public.whack_boss_week_start(p_ts timestamptz DEFAULT now())
RETURNS date
LANGUAGE sql
STABLE
...
CREATE TABLE IF NOT EXISTS public.whack_boss_rounds (
  ...
  hit_events    jsonb NOT NULL DEFAULT '[]'::jsonb,
  duration_ms   integer NOT NULL DEFAULT 18000 CHECK (duration_ms BETWEEN 10000 AND 30000),
...
ALTER TABLE public.whack_boss_rounds
  ADD COLUMN IF NOT EXISTS hit_events jsonb NOT NULL DEFAULT '[]'::jsonb;

CREATE TABLE IF NOT EXISTS public.whack_boss_scores (
  id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  round_id      uuid NOT NULL UNIQUE REFERENCES public.whack_boss_rounds(id) ON DELETE CASCADE,
  user_id       uuid NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  nick_snapshot text NOT NULL,
  ...
  duration_ms   integer NOT NULL DEFAULT 18000,
  submitted_at  timestamptz NOT NULL DEFAULT now(),
  client_meta   jsonb NOT NULL DEFAULT '{}'::jsonb
);

CREATE TABLE IF NOT EXISTS public.whack_boss_weekly_awards (
  id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  week_start    date NOT NULL,
  user_id       uuid NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  nick_snapshot text NOT NULL,
...
CREATE INDEX IF NOT EXISTS whack_boss_rounds_user_time_idx
  ON public.whack_boss_rounds(user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS whack_boss_rounds_expires_idx
  ON public.whack_boss_rounds(expires_at)
  WHERE submitted_at IS NULL;

CREATE INDEX IF NOT EXISTS whack_boss_scores_week_rank_idx
  ON public.whack_boss_scores(week_start, score DESC, accuracy DESC, submitted_at ASC);

CREATE INDEX IF NOT EXISTS whack_boss_scores_user_time_idx
  ON public.whack_boss_scores(user_id, submitted_at DESC);

CREATE INDEX IF NOT EXISTS whack_boss_awards_week_idx
  ON public.whack_boss_weekly_awards(week_start DESC, rank ASC);

ALTER TABLE public.whack_boss_rounds ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.whack_boss_scores ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.whack_boss_weekly_awards ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "whack_boss_rounds_select_own" ON public.whack_boss_rounds;
CREATE POLICY "whack_boss_rounds_select_own" ON public.whack_boss_rounds
  FOR SELECT TO authenticated USING (user_id = auth.uid());

DROP POLICY IF EXISTS "whack_boss_scores_select" ON public.whack_boss_scores;
CREATE POLICY "whack_boss_scores_select" ON public.whack_boss_scores
  FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "whack_boss_awards_select" ON public.whack_boss_weekly_awards;
CREATE POLICY "whack_boss_awards_select" ON public.whack_boss_weekly_awards
  FOR SELECT TO authenticated USING (true);

REVOKE ALL ON public.whack_boss_rounds, public.whack_boss_scores, public.whack_boss_weekly_awards
  FROM anon, authenticated;
GRANT SELECT ON public.whack_boss_rounds, public.whack_boss_scores, public.whack_boss_weekly_awards
  TO authenticated;
...
CREATE OR REPLACE VIEW public.whack_boss_recent_awards WITH (security_invoker = true) AS
SELECT
  id,
  week_start,
  user_id,
...
REVOKE ALL ON FUNCTION public.whack_boss_week_start(timestamptz) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.award_whack_boss_week(date) FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.whack_boss_week_start(timestamptz) TO authenticated;
GRANT SELECT ON public.whack_boss_current_week, public.whack_boss_all_time, public.whack_boss_recent_awards
  TO authenticated;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime'
      ...
      AND tablename = 'whack_boss_scores'
  ) THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.whack_boss_scores;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime'
      AND schemaname = 'public'
      AND tablename = 'whack_boss_weekly_awards'
  ) THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.whack_boss_weekly_awards;
  END IF;
END;
$$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'cron') THEN
    ...
    FROM cron.job
    WHERE jobname = 'whack_boss_weekly_awards';
